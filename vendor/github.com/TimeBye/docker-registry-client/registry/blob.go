...
func (registry *Registry) DownloadBlob(repository string, digest digest.Digest) (io.ReadCloser, error) {
	...
	glog.V(4).Infof("registry.blob.download url=%s repository=%s digest=%s", url, repository, digest)
...
func (registry *Registry) UploadBlob(repository string, digest digest.Digest, content io.Reader) error {
	...
	glog.V(4).Infof("registry.blob.upload url=%s repository=%s digest=%s", uploadUrl, repository, digest)
...
func (registry *Registry) HasBlob(repository string, digest digest.Digest) (bool, error) {
	...
	glog.V(4).Infof("registry.blob.check url=%s repository=%s digest=%s", checkUrl, repository, digest)
...
func (registry *Registry) initiateUpload(repository string) (*url.URL, error) {
	...
	glog.V(4).Infof("registry.blob.initiate-upload url=%s repository=%s", initiateUrl, repository)
